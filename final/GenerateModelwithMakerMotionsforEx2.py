...
# 実験 2 用
# 各教示動作の初期状態は完全ランダムではなく
# 直前の動作の初期状態から微妙に変化したもの
...
def generate(path, num):
    ...
    ginits = {"hand" : np.zeros(2)}
    for o in objList:
        ginits[o] = np.array([rands.pop(), rands.pop()])
    ...
    for i in range(num):
        ...
        # 初期値を微動
        rands = list((np.random.rand(objNum*2)-0.5) * 3000)
        inits = {"hand" : np.zeros(2)}
        for o in objList:
            inits[o] = ginits[o] + np.array([rands.pop(), rands.pop()])
...
if __name__ == "__main__":
    ...
    generate("tmp/log_ex2/", numTrain)
    # test  (Matching 学習用)
    generate("tmp/log_ex2_test/", numTest)
