...
data, samplerate = sf.read("001.wav")
timeSeries = []
...
sf.write("002.wav", [r[0] for r in result["prediction"]], samplerate)
...
for t, tRate in enumerate([2**i for i in range(tRange)]):
    ...
    plt.legend()
    sf.write("004_"+str(tRate)+".wav", [g[0] for g in gen], samplerate)
...
sf.write("003.wav", [g[0] for g in gen], samplerate)
"""
